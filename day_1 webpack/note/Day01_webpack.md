...
> 目标: webpack本身是, node的一个第三方模块包, 用于打包代码 默认只支持js文件，需要第三方包
...
> 目标: 把src下的2个js文件, 打包到1个js中, 并输出到默认dist目录下  注意src和dist这两个目录是默认的   注意要有package.json在同级src目录下 保证有那个自定义命令
...
3. 配置scripts(自定义命令) bulid 是自定义的命令名称  webpack是固定的 一个打包命令
...
> 总结: 万物皆模块, 引到入口, 才会被webpack打包, css打包进js(css-loader)中, 然后被嵌入在style(style-loader)标签插入dom上

### 3.6_加载器 - 处理less文件

> 目标: 1.less-loader让webpack处理less文件, 2.less模块翻译less代码
...
babel 的介绍 =>一个javascript编译器， 用于处理高版本 js语法 的兼容性  [babel官网](https://www.babeljs.cn/)
...
解决问题: 起一个开发服务器,  在电脑内存中打包, **缓存一些已经打包过的内容(也就是dist下)**, 只重新打包修改的文件, **最终运行加载在内存中给浏览器使用**

### ==4.1_webpack-dev-server自动刷新==

> 目标: 启动本地服务, 可实时更新修改的代码, 打包**变化代码**到内存中, 然后直接提供端口和网页访问  必须给index.html
