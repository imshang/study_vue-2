...
在vue中需要遵循单向数据流原则
    ...
    2. 子组件不能直接修改父组件传递过来的props  props是只读的(**毕竟儿子的数据是来自父亲的，父亲给的数据儿子是不能改的**)
...
比如：我要对一群儿子进行删除操作(不能让儿子进行删除操作，要把删除的儿子信息告诉儿子，儿子自动告诉父亲然后由父亲执行)，儿子要告诉父亲我是哪个儿子以及需要的数据 
...
* **父: @自定义事件名="父methods函数"(给儿子绑定了一个事件)**
...
两个组件的关系非常的复杂，通过父子组件通讯是非常麻烦的。这时候可以使用通用的组件通讯方案：事件总线（event-bus)    想办法找一个事件上共同的爹
