...
> 目标:  .vue文件分2类, 一个是**页面组件**, 一个是**复用组件**

**.vue文件本质无区别, 方便大家学习和理解, 总结的一个经验**

src/views(或pages) 文件夹(页面组件) 和 src/components文件夹(复用组件)
...
2. main.js– 继续配置2级路由
   ...
   **二级路由往后path直接写名字, 无需/开头**
   ...
   嵌套路由在**上级路由的children数组里编写路由信息对象**
...
**自己看文档吧 根据不同版本看不同api，以下笔记仅使用于vue2.**
